// composables/useWalletLock.js - MERGED VERSION
...
export const useWalletLock = () => {
   ...
   */
  const lockWallet = async (walletId, amount, reason, scheduledUnlock = null) => {
    loading.value = true
    error.value = null
   ...
   */
  const unlockWallet = async (walletId, lockId, reason) => {
    loading.value = true
    error.value = null
  ...
  /**
   * Get all wallet locks for current user
   */
  const getWalletLocks = async () => {
    ...
    try {
      ...
      const result = await $fetch('/api/wallet/locks')
  ...
  /**
   * Computed property to check if wallet is locked
   */
