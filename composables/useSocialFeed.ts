...
export const useSocialFeed = () => {
  ...
  // Proxy reference to support components reading authStore out of this hook natively
  ...
  const copyPostLink = async (postId: string) => {
    try {
      ...
      console.error('[Feed] Error copying link:', error)
    }
  }
  ...
  const triggerStatusViewer = (statusItem: any) => {
    activeSelectedStatus.value = statusItem
  }

  const closeStatusViewer = () => {
    activeSelectedStatus.value = null
  }
  ...
  const fetchPosts = async () => {
    try {
      ...
      const result: any = await fetchWithAuth(endpoint, {
        method: 'GET',
  ...
  const fetchProfileData = async () => {
    try {
      ...
      if (!profileStore.profile) {
        if (profileStore.profileMissing) {
      ...
      profileError.value = error.message || 'Failed to sync layout configuration.'
    } finally {
  ...
  // Graceful boot initialization layer for client lifecycle cycles
  if (process.client) {
    getAuthStore()
      .then(() => getProfileStore())
      .catch(() => console.warn('[Social Feed] Store layout hydration deferred.'))
  }
  ...
  return {
    ...
    userFollowers, userFollowing, userPosts, userStatus, authStore: authStoreProxy,
