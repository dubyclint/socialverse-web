        ...
        <div v-if="showAvatarUpload" class="modal-overlay" @click="showAvatarUpload = false">
          <div class="modal-content" @click.stop>
            ...
            <div class="modal-body">
              <ProfileAvatarUpload v-model="avatarUrlModel" />
              <div class="modal-actions">
                <button class="action-btn secondary" @click="showAvatarUpload = false">
                  Cancel
                </button>
                <button class="action-btn primary" @click="handleAvatarModelUpdate(avatarUrlModel)">
                  Save Avatar
                </button>
              </div>
            </div>
...
const loadUserProfile = async () => {
  ...
  try {
    ...
    avatarUrlModel.value = user.value.avatar || ''
    userStats.followers = Number(p.followers_count || 0)
    userStats.following = Number(p.following_count || 0)
    userStats.posts = Number(p.posts_count || 0)
  } catch (error) {
...
const handleAvatarModelUpdate = async (newUrl: string) => {
  avatarUrlModel.value = newUrl
  ...
  try {
    ...
    console.warn('[UserProfileCard] avatar persist warning:', e)
  }
}
