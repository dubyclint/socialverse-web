<template>
  <div class="bg-slate-800 rounded-lg shadow-xl p-8 border border-slate-700">
    ...
    <div v-if="localValidationError || authError" class="mb-4 p-4 bg-red-900/30 border border-red-500/70 rounded-lg">
      <div class="flex items-start gap-3">
        <svg class="w-5 h-5 text-red-400 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
          <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd" />
        </svg>
        <p class="text-red-300 text-sm font-medium">{{ localValidationError || authError }}</p>
      </div>
    </div>
    ...
    <form @submit.prevent="handleSubmit" class="space-y-4">
      ...
      <div>
        ...
        <p class="mt-1 text-xs text-slate-400">
          3-20 characters, lowercase letters, numbers, underscores, hyphens only
        </p>
    ...
    <p class="mt-6 text-center text-sm text-slate-400">
      ...
      <NuxtLink to="/signin" class="text-blue-500 hover:text-blue-400 font-medium">
        Sign in here
      </NuxtLink>
    </p>
  </div>
...
// Deconstruct composable hooks safely
...
const handleSubmit = async () => {
  ...
  try {
    console.log('[SignUpForm] Requesting account processing via signup composition hook...')
    
    // Pass user option blocks satisfying exact metadata configurations
    const result = await signUp(formData.email, formData.formData ? formData.password : formData.password, {
      data: {
    ...
    console.log('[SignUpForm] ✓ Registration cycle resolved complete')
    
    setTimeout(() => {
      emit('success', { email: formData.email, session: result?.session })
    }, 500)

  } catch (err: any) {
    console.error('[SignUpForm] ✗ Component sequence caught submission rejection:', err)
  }
}
</script>
